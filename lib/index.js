...
var AnimatedFlatList = react_native_reanimated_1.default.createAnimatedComponent(
  react_native_gesture_handler_1.FlatList
);
var Value = react_native_reanimated_1.default.Value,
  ...
  useCode = react_native_reanimated_1.default.useCode,
...
var DraggableFlatList = /** @class */ (function(_super) {
  ...
  function DraggableFlatList(props) {
    ...
    _this.horizontalSwipeHandlerRef = react_1.default.createRef();
    _this.draggableHandlerRef = react_1.default.createRef();
    _this.containerSize = new Value(0);
    _this.hsTouchInit = new Value(0); // Position of initial touch
    _this.dragTouchInit = new Value(0); // Position of initial touch
    _this.hsActivationDistance = new Value(0); // Distance finger travels from initial touch to when dragging begins
    _this.dragActivationDistance = new Value(0); // Distance finger travels from initial touch to when dragging begins
    _this.hsTouchAbsolute = new Value(0); // Finger position on screen, relative to container
    _this.dragTouchAbsolute = new Value(0); // Finger position on screen, relative to container
    _this.horizontalSwipeGestureState = new Value(
      react_native_gesture_handler_1.State.UNDETERMINED
    );
    _this.draggableGestureState = new Value(
      react_native_gesture_handler_1.State.UNDETERMINED
    );
    _this.isPressedIn = {
      native: new Value(0),
    ...
    _this.isScrolledUp = lessOrEq(
      sub(_this.scrollOffset, scrollPositionTolerance),
      0
    );
    _this.isScrolledDown = greaterOrEq(
      add(_this.scrollOffset, _this.containerSize, scrollPositionTolerance),
      _this.scrollViewSize
    );
    _this.touchCellOffset = sub(_this.dragTouchInit, _this.activeCellOffset); // Distance between touch point and edge of cell
    _this.hoverAnimUnconstrained = sub(
      sub(_this.dragTouchAbsolute, _this.dragActivationDistance),
      _this.touchCellOffset
    );
    _this.hoverAnimConstrained = min(
      sub(_this.containerSize, _this.activeCellSize),
      max(0, _this.hoverAnimUnconstrained)
    );
    ...
    _this.hoverAnimConfig = __assign(
      __assign(
        __assign({}, defaultAnimationConfig),
        _this.props.animationConfig
      ),
      { toValue: _this.hoverTo }
    );
    _this.distToTopEdge = max(0, _this.hoverAnim);
    _this.distToBottomEdge = max(
      0,
      sub(_this.containerSize, add(_this.hoverAnim, _this.activeCellSize))
    );
    ...
    _this.resetHoverSpring = [
      set(_this.dragTouchAbsolute, _this.hoverAnimConfig.toValue),
      set(_this.dragTouchInit, 0),
      set(_this.activeCellOffset, 0),
      set(_this.dragActivationDistance, 0),
      ...
      return __awaiter(_this, void 0, void 0, function() {
        ...
        return __generator(this, function(_a) {
          layoutInvalidationKeyHasChanged =
            prevProps.layoutInvalidationKey !==
            this.props.layoutInvalidationKey;
          ...
          if (!prevState.activeKey && this.state.activeKey) {
            ...
            if (cellData) {
              this.activeCellOffset.setValue(
                sub(cellData.offset, this.scrollOffset)
              );
    ...
    _this.resetHoverState = function() {
      ...
      _this.dragTouchAbsolute.setValue(0);
      _this.disabled.setValue(0);
      if (
        _this.state.hoverComponent !== null ||
    ...
    _this.setCellData = function(key, index) {
      ...
      var anim = procs_1.setupCell(
        ...
        onChangeSpacerIndex,
        onFinished,
        _this.isPressedIn.native,
        _this.placeholderOffset
      );
    ...
    _this.measureCell = function(key) {
      return new Promise(function(resolve, reject) {
        ...
        var flatListNode =
          _this.flatlistRef.current && _this.flatlistRef.current.getNode();
    ...
    _this.getScrollTargetOffset = function(
      distFromTop,
      distFromBottom,
      scrollOffset,
      isScrolledUp,
        ...
        return __generator(this, function(_a) {
          switch (_a.label) {
            ...
            case 2:
              ...
              (distFromTop = curParams[0]),
                (distFromBottom = curParams[1]),
                (scrollOffset = curParams[2]),
                (isScrolledUp = curParams[3]),
                (isScrolledDown = curParams[4]);
              targetOffset = this.getScrollTargetOffset(
                distFromTop,
                distFromBottom,
                scrollOffset,
                !!isScrolledUp,
    ...
    _this.isAtTopEdge = lessOrEq(
      _this.distToTopEdge,
      _this.props.autoscrollThreshold
    );
    _this.isAtBottomEdge = lessOrEq(
    ...
    _this.checkAutoscroll = cond(
      and(
        _this.isAtEdge,
        not(and(_this.isAtTopEdge, _this.isScrolledUp)),
        not(and(_this.isAtBottomEdge, _this.isScrolledDown)),
        eq(
          _this.draggableGestureState,
          react_native_gesture_handler_1.State.ACTIVE
        ),
        not(_this.isAutoScrollInProgress.native)
      ),
      call(_this.autoscrollParams, _this.autoscroll)
    );
    _this.onScroll = event([
      {
        nativeEvent: function(_a) {
          ...
          return block([
            set(
              _this.scrollOffset,
              _this.props.horizontal ? contentOffset.x : contentOffset.y
            ),
            cond(
              and(
                _this.isAutoScrollInProgress.native,
                or(
                  // We've scrolled to where we want to be
                  lessOrEq(
                    abs(sub(_this.targetScrollOffset, _this.scrollOffset)),
                    scrollPositionTolerance
                  ),
                  // We're at the start, but still want to scroll farther up
                  and(
                    _this.isScrolledUp,
                    lessOrEq(_this.targetScrollOffset, _this.scrollOffset)
                  ),
                  // We're at the end, but still want to scroll further down
                  and(
                    _this.isScrolledDown,
                    greaterOrEq(_this.targetScrollOffset, _this.scrollOffset)
                  )
                )
              ),
              [
    ...
    _this.onHorizontalSwipeStateChangeTester1 = function(event) {
      if (
        ...
        event.nativeEvent.state === react_native_gesture_handler_1.State.BEGAN
      ) {
        console.log("pan1 BEGAN");
      } else if (
        event.nativeEvent.state ===
        react_native_gesture_handler_1.State.CANCELLED
      ) {
        console.log("pan1 CANCELLED");
      } else if (
        event.nativeEvent.state === react_native_gesture_handler_1.State.ACTIVE
      ) {
        console.log("pan1 ACTIVE");
      } else if (
        event.nativeEvent.state === react_native_gesture_handler_1.State.END
      ) {
        console.log("pan1 END");
      }
    };
    _this.onHorizontalSwipeStateChangeTester2 = function(event) {
      if (
        event.nativeEvent.state ===
        react_native_gesture_handler_1.State.UNDETERMINED
      ) {
    ...
    _this.onHorizontalSwipeStateChange = event([
      {
        // get event data
        nativeEvent: function(_a) {
          var state = _a.state,
          ...
          return cond(neq(state, _this.horizontalSwipeGestureState), [
            ...
            cond(
              ...
              or(
                eq(state, react_native_gesture_handler_1.State.BEGAN), // Called on press in on Android, NOT on ios! GestureState.BEGAN may be skipped on fast swipes
                and(
                  eq(state, react_native_gesture_handler_1.State.ACTIVE),
                  neq(
                    _this.horizontalSwipeGestureState,
                    react_native_gesture_handler_1.State.BEGAN
                  )
                )
              ),
              ...
              [
                set(_this.hsTouchAbsolute, x),
                set(_this.hsTouchInit, _this.hsTouchAbsolute)
              ]
            ),
            ...
            cond(eq(state, react_native_gesture_handler_1.State.ACTIVE), [
              ...
              set(_this.hsActivationDistance, sub(x, _this.hsTouchInit)),
              // Assigns x value to hsTouchAbsolute
              set(_this.hsTouchAbsolute, x)
            ]),
            // checks if horizontalSwipeGestureState !== state
            cond(
              neq(_this.horizontalSwipeGestureState, state),
              // Assigns state value to horizontalSwipeGestureState
              set(_this.horizontalSwipeGestureState, state)
            ),
            // checks if state === END, CANCELLED, or FAILED
            cond(
              or(
                eq(state, react_native_gesture_handler_1.State.END),
                eq(state, react_native_gesture_handler_1.State.CANCELLED),
                eq(state, react_native_gesture_handler_1.State.FAILED)
              ),
              call([], function() {
                _this.hsTouchAbsolute.setValue(0);
                _this.hsTouchInit.setValue(0);
                _this.hsActivationDistance.setValue(0);
              })
            )
          ]);
        }
    ...
    _this.onHorizontalSwipeEvent = event([
      {
        // get event data
        nativeEvent: function(_a) {
          var x = _a.x;
    ...
    //     // console.log(event.nativeEvent.translationX);
    // };
    _this.onDraggableStateChange = event([
      {
        // get event data
        nativeEvent: function(_a) {
          var state = _a.state,
          ...
          return cond(
            and(neq(state, _this.draggableGestureState), not(_this.disabled)),
            [
              // checks to see if "or()" returns truthy
              cond(
                // checks to see if either node is truthy
                ...
                [
                  set(_this.dragTouchAbsolute, _this.props.horizontal ? x : y),
                  set(_this.dragTouchInit, _this.dragTouchAbsolute)
                ]
              ),
              // checks to see that state === ACTIVE
              cond(eq(state, react_native_gesture_handler_1.State.ACTIVE), [
                ...
                set(
                  _this.dragActivationDistance,
                  sub(_this.props.horizontal ? x : y, _this.dragTouchInit)
                ),
                // Assigns y value to dragTouchAbsolute
                set(_this.dragTouchAbsolute, _this.props.horizontal ? x : y)
              ]),
              ...
              cond(
                neq(_this.draggableGestureState, state),
                // Assigns state value to panGestureState
                set(_this.draggableGestureState, state)
              ),
              // checks if state === END, CANCELLED, or FAILED
              cond(
                or(
    ...
    _this.onDraggableGestureEvent = event([
      {
        // get event data
        nativeEvent: function(_a) {
          var x = _a.x,
    ...
    _this.hoverComponentTranslate = cond(
      clockRunning(_this.hoverClock),
      _this.hoverAnimState.position,
      _this.hoverAnim
    );
    _this.hoverComponentOpacity = and(
      _this.isHovering,
      neq(
        _this.draggableGestureState,
        react_native_gesture_handler_1.State.CANCELLED
        ...
        react_native_reanimated_1.default.View,
        {
          style: [
              ...
              : styles.hoverComponentVertical,
            {
              ...
              transform: [
                ((_a = {}),
                (_a["translate" + (horizontal ? "X" : "Y")] =
                  _this.hoverComponentTranslate),
                _a)
              ]
            }
          ]
      ...
      return react_1.default.createElement(
        ...
        function() {
          return block([
            onChange(
              _this.spacerIndex,
              call([_this.spacerIndex], function(_a) {
    ...
    _this.renderPlaceholder = function() {
      ...
      var style = __assign(
        __assign({}, react_native_1.StyleSheet.absoluteFillObject),
        ((_a = {}),
        (_a[sizeKey] = _this.activeCellSize),
        (_a.transform = [
  ...
  DraggableFlatList.prototype.render = function() {
    ...
    var _a = this.props,
      ...
      dragActivationDistance = _a.dragActivationDistance,
    ...
    if (dragActivationDistance) {
      var activeOffset = [-dragActivationDistance, dragActivationDistance];
          ...
          react_1.default.createElement(
            ...
            react_1.default.createElement(
              ...
              function() {
                return block([
                  onChange(
                    _this.isPressedIn.native,
                    cond(not(_this.isPressedIn.native), _this.onGestureRelease)
                  ),
            ...
            react_1.default.createElement(
              ...
              function() {
                return block([
                  onChange(
                    _this.hsActivationDistance,
                    call([_this.hsActivationDistance], function(_a) {
                      var hsActivationDistance = _a[0];
                      console.log(
            ...
            react_1.default.createElement(
              ...
              function() {
                return block([
                  onChange(
                    _this.horizontalSwipeGestureState,
                    call([_this.horizontalSwipeGestureState], function(_a) {
                      var horizontalSwipeGestureState = _a[0];
                      console.log(
            ...
            onScrollOffsetChange &&
              react_1.default.createElement(
                ...
                function() {
                  return onChange(
                    _this.scrollOffset,
                    call([_this.scrollOffset], function(_a) {
                      var offset = _a[0];
