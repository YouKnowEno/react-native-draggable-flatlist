...
var DraggableFlatList = /** @class */ (function(_super) {
  ...
  function DraggableFlatList(props) {
    ...
    _this.panGestureState = new Value(
    ...
    _this.hoverAnimUnconstrained = sub(
      sub(_this.touchAbsolute, _this.activationDistance),
    ...
    _this.checkAutoscroll = cond(
      and(
        ...
        eq(_this.panGestureState, react_native_gesture_handler_1.State.ACTIVE),
    ...
    _this.onPanStateChange = event([
      {
        nativeEvent: function(_a) {
          var state = _a.state,
            ...
            y = _a.y;
          return cond(
            and(neq(state, _this.panGestureState), not(_this.disabled)),
            [
              cond(
                //  Will
                or(
                  eq(state, react_native_gesture_handler_1.State.BEGAN), // Called on press in on Android, NOT on ios!
                  // GestureState.BEGAN may be skipped on fast swipes
                  and(
                    eq(state, react_native_gesture_handler_1.State.ACTIVE),
                    neq(
                      _this.panGestureState,
              ...
              cond(eq(state, react_native_gesture_handler_1.State.ACTIVE), [
                set(
                  ...
                  sub(_this.props.horizontal ? x : y, _this.touchInit)
                  // Calc distance between touchInit and absolute position of x/y
                ),
                set(_this.touchAbsolute, _this.props.horizontal ? x : y)
              ]),
              cond(
                neq(_this.panGestureState, state),
    ...
    _this.hoverComponentOpacity = and(
      _this.isHovering,
      neq(_this.panGestureState, react_native_gesture_handler_1.State.CANCELLED)
    );
    ...
    //     // console.log(event.nativeEvent.translationX);
    // };
    _this.onHorizontalSwipeStateChange = function(event) {
      if (
    ...
    _this.onHorizontalSwipeEvent = function(event) {
      set(_this.touchAbsolute, event.nativeEvent.x);
      set(_this.touchInit, _this.touchAbsolute);
      console.log("distance");
      console.log(_this.touchInit - event.nativeEvent.x);
      // console.log("event.nativeEvent.x");
      // console.log(event.nativeEvent.x);
      // console.log("event.nativeEvent.absoluteX");
      // console.log(event.nativeEvent.absoluteX);
