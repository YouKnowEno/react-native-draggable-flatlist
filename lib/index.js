...
var DraggableFlatList = /** @class */ (function(_super) {
  ...
  function DraggableFlatList(props) {
    ...
    _this.panGestureHandlerRef2 = react_1.default.createRef();
    _this.containerSize = new Value(0);
    _this.touchInit1 = new Value(0); // Position of initial touch
    _this.touchInit2 = new Value(0); // Position of initial touch
    _this.activationDistance = new Value(0); // Distance finger travels from initial touch to when dragging begins
    _this.touchAbsolute1 = new Value(0); // Finger position on screen, relative to container
    _this.touchAbsolute2 = new Value(0); // Finger position on screen, relative to container
    _this.panGestureState1 = new Value(
      react_native_gesture_handler_1.State.UNDETERMINED
    );
    _this.panGestureState2 = new Value(
      react_native_gesture_handler_1.State.UNDETERMINED
    );
    _this.isPressedIn = {
      native: new Value(0),
    ...
    _this.touchCellOffset = sub(_this.touchInit2, _this.activeCellOffset); // Distance between touch point and edge of cell
    _this.hoverAnimUnconstrained = sub(
      sub(_this.touchAbsolute2, _this.activationDistance),
    ...
    _this.resetHoverSpring = [
      set(_this.touchAbsolute2, _this.hoverAnimConfig.toValue),
      set(_this.touchInit2, 0),
    ...
    _this.resetHoverState = function() {
      ...
      _this.touchAbsolute2.setValue(0);
    ...
    _this.checkAutoscroll = cond(
      and(
        ...
        eq(_this.panGestureState2, react_native_gesture_handler_1.State.ACTIVE),
    ...
    _this.onHorizontalSwipeStateChange = event([
      {
        ...
        nativeEvent: function(_a) {
          ...
          return cond(neq(state, _this.panGestureState1), [
            ...
            cond(
              ...
              [
                set(_this.touchAbsolute1, _this.props.horizontal ? x : y),
                set(_this.touchInit1, _this.touchAbsolute1)
              ]
            ),
            ...
            cond(eq(state, react_native_gesture_handler_1.State.ACTIVE), [
              ...
              set(
                ...
                sub(_this.props.horizontal ? x : y, _this.touchInit1)
              ),
              // Assigns y value to touchAbsolute
              set(_this.touchAbsolute1, _this.props.horizontal ? x : y)
            ]),
            ...
            cond(
              neq(_this.panGestureState1, state),
              // Assigns state value to panGestureState
              set(_this.panGestureState1, state)
            ),
    ...
    _this.onPanStateChange = event([
      {
        ...
        nativeEvent: function(_a) {
          ...
          return cond(
            and(neq(state, _this.panGestureState2), not(_this.disabled)),
            [
              // checks to see if "or()" returns truthy
              cond(
                // checks to see if either node is truthy
                ...
                // Assigns touchAbsolute value to touchInit
                [
                  set(_this.touchAbsolute2, _this.props.horizontal ? x : y),
                  set(_this.touchInit2, _this.touchAbsolute2),
                  // block([call([], () => console.log(this.touchAbsolute2))]),
                  // block([call([], () => console.log(this.touchInit2))]),
                  block([
                    call([], function() {
                      return console.log("run run running");
                    })
                  ])
                ]
              ),
              ...
              cond(eq(state, react_native_gesture_handler_1.State.ACTIVE), [
                ...
                set(
                  _this.activationDistance,
                  sub(_this.props.horizontal ? x : y, _this.touchInit2)
                ),
                // Assigns y value to touchAbsolute
                set(_this.touchAbsolute2, _this.props.horizontal ? x : y)
              ]),
              // checks if panGestureState /== state
              cond(
                neq(_this.panGestureState2, state),
                // Assigns state value to panGestureState
                set(_this.panGestureState2, state)
              ),
              // checks if state === END, CANCELLED, or FAILED
              cond(
                or(
        ...
        react_1.default.createElement(
          ...
          react_1.default.createElement(
            ...
            function() {
              return block([
                onChange(
                  _this.touchAbsolute2,
                  call([_this.touchAbsolute2], function(touchAbsolute2) {
                    console.log("touchAbsolute: " + touchAbsolute2[0]);
                  })
                )
              ]);
            }
